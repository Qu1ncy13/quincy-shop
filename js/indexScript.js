...
function renderTopItems() {
    ...
    topItems.forEach((item) =>{
        topItemsHTML += `
            <div class="product-card" data-id="${item.id}">
                <a href="html/product.html?id=${item.id}"><img src="${item.image}" alt="${item.alt}"></a>
                <a href="html/product.html?id=${item.id}" class="product-card-name"><h3>${item.title}</h3></a>                        
                <p>$ ${item.price}</p>
                <div class = "button-block">
                    <button class="catalog-btn add-cart-btn" data-id="${item.id}">Добавить в корзину</button>
                    <button class="add-favorites-btn" data-id="${item.id}">
    ...
    topProducts.innerHTML += topItemsHTML;
}
